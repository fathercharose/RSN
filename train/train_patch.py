from model_patch_same_size import DRSN_CS
from dataset_Maxmin import MyDataset_patch_normalization_Maxmin         # 可用下一行替换，来判断Maxmin Normalization与Absolute Maximum normalization的区别
# from dataset import MyDataset_patch_normalization                 #It uses Absolute Maximum normalization
...
# 划分数据集，训练集：验证集 = 8:1
full_dataset = MyDataset_patch_normalization_Maxmin(train_path_x, train_path_y)
# valida_size = 100
...
# 划分数据集
train_dataset, valida_dataset = torch.utils.data.random_split(full_dataset, [train_size, valida_size])
...
for epoch in range(epochs):
    ...
    train_loss = train_loss / (batch_idx1 + 1)  # 本次epoch的平均loss # Average loss of this epoch
