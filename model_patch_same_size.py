...
class DRSN_CS(nn.Module):
    def __init__(self, upscale_factor=2):
        ...
        self.body = nn.Sequential(
            ...
            nn.ReLU(inplace=True),  # 非线性激活函数ReLU
            ShrinkageMapping(in_channels=128, out_channels=128),  # 残差收缩模块
            ...
            nn.ReLU(inplace=True),  # 非线性激活函数ReLU
            ShrinkageMapping(in_channels=256, out_channels=256),  # 残差收缩模块
        ...
        self.upscale = nn.Sequential(
            ...
            nn.BatchNorm2d(128),  # 批标准化层
            nn.ReLU(inplace=True),  # 非线性激活函数ReLU
            ShrinkageMapping(in_channels=128, out_channels=128),

            nn.Conv2d(in_channels=128, out_channels=64, kernel_size=3, stride=1, padding=1, bias=True),
            nn.BatchNorm2d(64),  # 批标准化层
            nn.ReLU(inplace=True),  # 非线性激活函数ReLU
            ShrinkageMapping(in_channels=64, out_channels=64),
